...
def gen_erdos_renyi(uniform_gen, n_graphs, n_nodes, save_fp=None, directed=False):
    ...
    """
    edge_prob = min(math.log(float(n_nodes), 2)/float(n_nodes),
                    0.5)  # uniform_gen.sample((n_graphs,)) * 0.9 + 0.1 # we don't want empty graphs
    if directed:
        cutoff = edge_prob
    else:
    ...
    if directed:
        ...
        rand_sym_adj = torch.logical_and(
            rand_cutoff_adj, rand_cutoff_adj.transpose(1, 2))
    # ensuring there are no self-loops
    rand_sym_adj[:, torch.arange(n_nodes), torch.arange(n_nodes)] = 0

    # saving the tensor
    if save_fp is not None:
        torch.save(rand_sym_adj, save_fp+"erdosrenyi" +
                   str(n_graphs) + "_" + str(n_nodes) + ".pt")
...
def gen_barabasi_albert(uniform_gen, n_graphs, n_nodes, m_attach, save_fp=None):
    ...
    for i in range(n_nodes-m_attach):
        ...
        for j in range(m_attach):
            ...
            idx = n_nodes - \
                (probs_cdf > rand_src[:, i, j].unsqueeze(1)).sum(dim=1)
    ...
    # saving the tensor
    if save_fp is not None:
        torch.save(adj, save_fp+"barabasialbert" +
                   str(n_graphs) + "_" + str(n_nodes) + ".pt")
...
def gen_twod_grid(uniform_gen, n_graphs, n_nodes, save_fp=None, directed=False):
    ...
    shapes = list(
        set([sorted_2tuple(prod(seq), n_nodes // prod(seq)) for seq in factors]))
    if len(shapes) > 1:
        shapes = list(filter(lambda t: t[0] != 1, shapes))

    grid_shape = torch.argmax(uniform_gen.sample(
        (n_graphs, len(shapes))), dim=-1)

    # generate position of each node
    xy = [[(i, j, *shapes[idx]) for i in range(shapes[idx][0])
           for j in range(shapes[idx][1])] for idx in grid_shape.tolist()]
    neighbours = [[four_neighbours(*t) for t in g] for g in xy]
    values = [[[0 if i < 0 else 1 for i in node]
               for node in g] for g in neighbours]
    neighbours = [[[i if n == -1 else n for n in g[i]]
                   for i in range(len(g))] for g in neighbours]
    ...
    for i in range(n_nodes):
        ...
        perm_matrix[:, i, :] = 0
        perm_matrix.scatter_(
            2, idx.unsqueeze(-1).unsqueeze(-1).repeat(1, n_nodes, 1), 0)
    ...
    if save_fp is not None:
        torch.save(adj, save_fp+"twodgrid" +
                   str(n_graphs) + "_" + str(n_nodes) + ".pt")
